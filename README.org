...
(asahi-operating-system #:efi-uuid "9FBE-130E")
#+end_example
...
#+begin_src sh :results verbatim
  guix time-machine --channels=asahi/guix/channels.scm -- \
       system image --load-path=. --image-type=efi-raw asahi/guix/system/install.scm
#+end_src

Write the installer disk image to the USB stick at =/dev/sda=.

#+begin_src sh :results verbatim
  sudo dd if=/gnu/store/4cia1isqspzp1mz7qyqx4ay499cvhlq5-disk-image \
